    ...
    <div class="content" style="margin-top:15px;text-align:left;">
      <div class="content-left" style="height:100%;width:30%;float:left;">
          ...
          <div class="content-left-bottom" style="height:55%;background-color:#fff;margin-top:5%;">
              ...
              <tr style="height:50px;">
                <td>
                  最新测量信息: <span style="margin-left: 45%;"  @click="dialogFormVisible_1 = true"><el-button type="text"><i class="el-icon-document"></i>  编辑</el-button></span>
                </td>
                <el-dialog title="诊前测量" :visible.sync="dialogFormVisible_1">
                  <el-form :model="form"  label-width="80px">

                    <el-form-item label="患者:"  class="la-span">
                      <span v-bind="value1">{{form.name}} </span>
                      <span v-bind="value1">{{form.date}} </span>
                      <span v-bind="value1">{{form.time}} </span>
                      <span v-bind="value1">{{form.doctor}} </span>
                    </el-form-item>
              ...
              <tr style="height:50px;" v-bind="formData">
                <td>身高: <span v-bind="formData.height">{{ formData.height }}</span></td>
              </tr>

              <tr style="height:50px;">
                <td>体重: <span v-bind="formData.weight">{{ formData.weight }}</span></td>
              </tr>

              <tr style="height:50px;">
                <td>体温: <span v-bind="formData.temper">{{ formData.temper }}</span></td>
              </tr>

              <tr style="height:50px;">
                <td>血压: <span v-bind="formData.pressure">{{ formData.pressure }}</span></td>
              </tr>

              <tr style="height:50px;">
                <td>心率: <span v-bind="formData.rate">{{ formData.rate }}</span></td>
              </tr>
            </table>
          </div>
      </div>
      <div class="content-right" style="height:100%;width:60%;margin-left:30px;float:left;">
          <div class="content-right-top" style="height:43%;background-color:#fff;">
              <table border="0" cellspacing="0" cellpadding="0" style="width:100%;text-align: left;" >
                ...
                <tr style="height:50px;" v-bind="formData1">
                  <td style="font-weight: bold;">预约信息</td>
                </tr>
                <tr style="height:50px;">
                  <td>订单日期:
                      <span v-bind="form.value_res">
                        {{form.value_res}}
                      </span>
                    </td>
                </tr>
                ...
                <tr style="height:50px;">
                  <td>订单生成时间: <span v-bind="form.time">{{form.time}}</span></td>
                </tr>

                <tr style="height:50px;">
                  <td>预约类型: <span v-bind="form.doc_subject">{{form.doc_subject}}</span></td>
                </tr>

                <tr style="height:50px;">
                  <td>心率: <span v-bind="formData.rate">{{form.rate}}</span><span style="margin-left: 80%;"><el-button type="text"><i class="el-icon-document"></i>  查看报备</el-button></span></td>
                </tr>
          ...
          <div class="content-right-bottom" style="height:37%;background-color:#fff;margin-top:3%;">
              ...
              <tr style="height:50px;" v-bind="formData2">
                <td style="font-weight: bold;">问诊信息</td>
              </tr>

              <tr style="height:50px;">
                <td>就诊时间:
                  <span v-bind="form.timereservate">{{form.timeDate}}</span>
                  <span v-bind="form.timereservate">{{form.timeTime}}</span>
                </td>
              </tr>

              <tr style="height:50px;">
                <td>就诊医生: <span v-bind="form.doctor">{{form.doctor}}</span></td>
              </tr>

              <tr style="height:50px;">
                <td>主治: <span v-bind="form.doc_subject">{{form.doc_subject}}</span></td>
              </tr>
            </table>
          </div>
                ...
                <el-button type="success" @click="editorDialogForm">修改就诊时间</el-button>
                    ...
                    <el-dialog title="修改预约时间" :visible.sync="dialogFormVisible">
                      <el-form :model="form2">
                        ...
                        <el-form-item label="患者:"  class="la-span">
                            <span v-bind="form.name">{{form.name}} </span>
                            <span v-bind="form.timeDate">
                                {{ form.timeDate }}
                            </span>
                            <span v-bind="form.time">{{form.timeTime}} </span>
                            <span v-bind="form.doctor">{{form.doctor}} </span>
                        </el-form-item>
